...
export const COMPILER_CHECKS = {
  ...
  warnings: defineChecks({
    ...
    SLOT_RELOCATED: {
      ...
      regime: 'warning',
      /**
       * `quality` when the yield cost the scene data, `info` otherwise. A relocation that
       * merely moves a badge is information; one that halves a chart's categories is not,
       * and reporting both at the same volume is what let the second hide behind the first.
       */
      severity: ['info', 'quality'],
    ...
    CAPACITY_REDUCED_BY_COMPOSITION: {
      ...
      severity: ['quality'],
      means:
        'A scene yielded into a composition that holds fewer values than the full canvas, ' +
        'so the weakest were collapsed and are not on screen.',
      repair:
        'Free the region the scene occupies by placing the contending element in a slot ' +
        'the scene does not use, choose a layout the composition holds more of, or shorten ' +
        'the series to what the composition holds.',
    },
