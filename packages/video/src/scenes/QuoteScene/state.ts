 ...
 * read and no fourth copy of it is needed. This capability deliberately does not force the
 * extraction question that the three existing `state.ts` files have already argued.
 */
export type QuoteSceneState = {
  /** Frame the quote unit begins its entrance; `null` while the plan still holds it back. */
  quoteFrame: number | null;
};

export const REVEAL_QUOTE_ACTION = 'revealQuote';

/**
 * The quote auto-reveals when the instance carries no reveal event, so an example without
 * events still animates. As soon as the plan drives the reveal, the plan owns it.
 */
export const initialQuoteSceneState = (events: TimedEvent[]): QuoteSceneState => ({
  quoteFrame: events.some((e) => e.action === REVEAL_QUOTE_ACTION) ? null : 0,
});

export const quoteReducer: EventReducer<QuoteSceneState> = (state, event) => {
  switch (event.action) {
    case REVEAL_QUOTE_ACTION:
      return { ...state, quoteFrame: event.frame };
    default:
