...
async function main() {
  ...
  if (!origin || !jobId || !output)
    throw new Error('Usage: node deploy/studio/rehearse.cjs <origin> <job> <output-directory>');
  ...
  try {
    ...
    await login
      ...
      .fill(
        fs.readFileSync('.scratch/hackathon-launch/runtime/studio/access-code.txt', 'utf8').trim(),
      );
    ...
    await expect
      .poll(() => video.evaluate((v) => v.currentTime), { timeout: 30000 })
      .toBeGreaterThan(1);
    await video.evaluate((v) => {
      v.currentTime = Math.min(23, v.duration / 2);
    });
    await expect
      .poll(
        () => video.evaluate((v) => !v.seeking && v.currentTime >= Math.min(23, v.duration / 2)),
        { timeout: 30000 },
      )
      .toBe(true);
    ...
    await anon.close();
    const range = await recording.request.get(origin + job.preview.url, {
    ...
    await expect(
      page.getByText('Connection interrupted. Your work continues on the server. Reconnecting…'),
