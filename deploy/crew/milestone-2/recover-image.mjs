...
const providerRequest = { prompt: request.prompt, aspectRatio: request.aspectRatio,
  outputMimeType: request.outputMimeType, seed: request.seed };
...
assert(timingSafeEqual(Buffer.from(grant, 'hex'), createHmac('sha256', process.env.VOX_GRANT_KEY)
  .update(canonicalJson(unsigned)).digest()));
const store = new RunStore({ runRoot: '/var/lib/vox/runs/run-5dbd654e5afb',
  runId, ledgerRoot: process.env.VOX_LEDGER_ROOT, hmacKey: process.env.VOX_RUN_HMAC_KEY,
  keyId: process.env.VOX_RUN_KEY_ID, leaseMs: 300_000 });
...
if (mode === '--check') {
  console.log(JSON.stringify({ ready: true, runId, jobId, requestSha256, model,
    originalGrantRetained: true, maximumRecoveryDispatches: 1 }));
} else {
  await store.exclusive(async session => {
    let checkpoint = await session.inspect();
    const previousJob = inspect(checkpoint);
    const evidence = { runId, jobId, requestSha256, model, location: 'global',
      reason: 'Explicit operator recovery after retired Imagen endpoint produced no candidate.',
      previousJob, dispatchedAt: new Date().toISOString() };
    // Durable, exclusive marker: a repeated invocation cannot spend again, even after a crash.
    await writeFile(`${directory}/milestone-2-image-recovery-dispatch.json`,
      JSON.stringify(evidence), { flag: 'wx', mode: 0o600 });
    const replace = job => ({ ...checkpoint.bindings, images: { ...checkpoint.bindings.images,
      jobs: checkpoint.bindings.images.jobs.map(item => item.id === jobId ? job : item) } });
    checkpoint = await session.commit({ expectedRevision: checkpoint.revision,
      command: 'run.image.start', outcome: 'succeeded',
    ...
    let generated;
    try {
      generated = await createGoogleImageAdapter({ model,
        environment: { ...process.env, GOOGLE_CLOUD_LOCATION: 'global' } }).generate(providerRequest);
      ...
      const details = { ...evidence, outcome: 'uncertain', errorName: error?.name,
        httpStatus: typeof error?.status === 'number' ? error.status : null };
      await writeFile(`${directory}/milestone-2-image-recovery-outcome.json`, JSON.stringify(details));
      await session.commit({ expectedRevision: checkpoint.revision, command: 'run.image.start',
        outcome: 'succeeded', bindings: replace({ ...previousJob, status: 'uncertain',
    ...
    assert(png.length >= 24 && png.subarray(0, 8).equals(Buffer.from([137,80,78,71,13,10,26,10])));
    assert.equal(png.toString('ascii', 12, 16), 'IHDR');
    const width = png.readUInt32BE(16), height = png.readUInt32BE(20);
    assert(width > 0 && height > 0);
    const sha256 = hash(png);
    const result = await session.commit({ expectedRevision: checkpoint.revision,
      command: 'run.image.start', outcome: 'succeeded',
      artifacts: [{ kind: 'generated_image_candidate',
        path: RUN_PATHS.generatedImageCandidate(jobId, sha256), bytes: png }],
      bindings: ([artifact]) => replace({ ...previousJob, status: 'candidate', failure: null,
        candidate: { id: `image-candidate-${sha256.slice(0,20)}`,
          requirementId: request.requirementId, identityKey: request.identityKey,
          promptSha256: hash(request.prompt), artifact, width, height } }),
      data: { operatorRecovery: true, model, requestSha256, candidateSha256: sha256 } });
    const outcome = { ...evidence, outcome: 'candidate', sha256, width, height,
      revision: result.revision, completedAt: new Date().toISOString() };
    await writeFile(`${directory}/milestone-2-image-recovery-outcome.json`, JSON.stringify(outcome));
