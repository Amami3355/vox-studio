...
const input = { runId: '11111111-1111-4111-8111-111111111111', request: {
  protocolVersion: 1, requirementId: 'req_12345678', identityKey: 'test-illustration',
  prompt: 'An illustration for a test.', aspectRatio: '16:9', outputMimeType: 'image/png',
  seed: 7, requestSha256: 'a'.repeat(64),
} };
const request = '/tmp/test-image-request.json';
const output = '/tmp/test-image-grant.json';
const run = () => spawnSync('/app/packages/production/node_modules/.bin/tsx',
...
assert.equal(signature, createHmac('sha256', 'synthetic-test-key').update(canonicalJson(unsigned)).digest('hex'));
assert.ok(!first.stdout.includes(signature));
assert.equal(run().status, 0);
assert.deepEqual(JSON.parse(await readFile(output, 'utf8')), grant);
await writeFile(request, JSON.stringify({ ...input, runId: '22222222-2222-4222-8222-222222222222' }));
assert.notEqual(run().status, 0);
assert.deepEqual(JSON.parse(await readFile(output, 'utf8')), grant);
console.log('passed: exact-request signature, idempotent grant, second-request refusal, secret-free output');
