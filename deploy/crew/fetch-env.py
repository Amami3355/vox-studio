...
def main():
    parallel = sys.argv[1:] == ["--parallel"]
    if sys.argv[1:] and not parallel:
        raise ValueError("Only --parallel is supported.")
    secret = "PARALLEL_API_KEY" if parallel else "VOX_NETWORK_TOKEN"
    ...
    payload = json.loads(read(
        f"https://secretmanager.googleapis.com/v1/projects/{project}/secrets/{secret}/versions/latest:access",
    ...
    os.umask(0o077)
    path = Path("/runtime/parallel.env" if parallel else "/runtime/network.env")
    ...
    print(secret + " installed at mode 0600; no Production private secrets requested.")
