...
register anything, and no `switch` on a capability id anywhere in the render path.
...
| every Remotion composition | `src/Root.tsx:111` | one per example, from the registry |
