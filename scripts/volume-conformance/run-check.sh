...
# stderr. This keeps only what the check printed.
...
TOOLING="$(on_vm 'echo "user=$(id -un) uid=$(id -u)"; echo "os=$(grep ^PRETTY_NAME= /etc/os-release | cut -d= -f2- | tr -d \")"; for t in docker mkfs.ext4 mount mountpoint lsblk blkid; do printf "%s=%s\n" "$t" "$(command -v $t 2>/dev/null || echo missing)"; done' 2>/dev/null || true)"
...
if [[ "$CONTROL_ONLY" == "yes" ]]; then
  ...
  DISK_REPORT="$(on_vm "sudo bash ~/${REMOTE_DIR}/prepare-disk.sh ${DISK} ${MOUNT_POINT}" 2>&1 || true)"
...
if [[ "$CONTROL_ONLY" == "yes" ]]; then
  ...
  RUN_CMD="sudo docker run --rm -v ${MOUNT_POINT}:${IN_CONTAINER} -v \$HOME/${REMOTE_DIR}/check.mjs:/check.mjs:ro ${IMAGE} node /check.mjs ${IN_CONTAINER} ${CHECK_ARGS}"
  set +e
...
if [[ "$SKIP_CONTROL" == "yes" ]]; then
  ...
  if confirm "Create the control bucket and run it?"; then
    ...
    on_vm "sudo docker build -q -f ~/${REMOTE_DIR}/negative-control.Dockerfile -t ${CONTROL_IMAGE} ~/${REMOTE_DIR}" >/dev/null \
